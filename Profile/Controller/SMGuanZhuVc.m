...
@interface SMGuanZhuVc ()
@property(nonatomic,strong)NSMutableArray *modelArr ;
@end

@implementation SMGuanZhuVc
...
-(void)getFollowsList
{
    ...
    if (self.userModel.user_id) {
        strUrl = [NSString stringWithFormat:@"http://club.dituhui.com/api/v2/users/%@/followings",self.userModel.user_id];
    }else
    {
        SMAccount *acount = [SMAccount accountFromSandbox];
         strUrl = [NSString stringWithFormat:@"http://club.dituhui.com/api/v2/users/%@/followings",acount.user_id];
    }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (self.modelArr.count == 0) {
        vHeader.textLabel.text = @"还没有关注任何人";
    }else{
        vHeader = nil;
    }
