...
//    UIView *sourceView = self.sourceImagesContainerView.subviews[self.currentImageIndex];
    UIView *parentView = [self getParsentView:_sourceView];
    CGRect rect = [_sourceView.superview convertRect:_sourceView.frame toView:parentView];
...
//        tempImageView.frame = targetTemp;
        tempImageView.frame = CGRectMake(0, 0, kWidth, kHeight);
...
//    [[self getCurrentVC] presentViewController:self animated:NO completion:nil];
}
...
- (void)hidePhotoBrowser:(UITapGestureRecognizer *)recognizer
{
    ...
    UIView *parentView = [self getParsentView:_sourceView];
    CGRect targetTemp = [_sourceView.superview convertRect:_sourceView.frame toView:parentView];
//    CGRect targetTemp = currentImageView.frame;
    // 减去偏移量
//    if ([sourceView.superview isKindOfClass:[UICollectionView class]]) {
//        UITableView *tableview = (UITableView *)sourceView.superview.superview.superview.superview.superview;
...
{
    int autualIndex = scrollView.contentOffset.x  / kWidth ;
    //设置当前下标
    self.currentImageIndex = autualIndex;
    
    if ([self.delegate respondsToSelector:@selector(cellChangeL:)]) {
        [self.delegate cellChangeL:autualIndex];
    }
    
        _sourceView = self.sourceImagesContainerView;
//    _sourceView = _sourceView.superview.subviews[autualIndex];
